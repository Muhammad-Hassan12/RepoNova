            ...
            <div className="backdrop-blur-2xl bg-black/80 sm:bg-black/60 border-t sm:border border-white/20 sm:border-white/10 rounded-t-3xl sm:rounded-2xl shadow-[0_0_30px_rgba(255,136,0,0.2)] overflow-hidden">
                ...
                <div className="p-5 border-b border-white/10" style={{ borderTopColor: body.color, borderTopWidth: 3 }}>
                    <div className="flex justify-between items-start">
                        <div>
                            <h2 className="text-xl font-bold text-white tracking-wide">{body.repo_name}</h2>
                ...
                <div className="p-5 grid grid-cols-3 gap-3">
                    <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                        ...
                        <div className="text-yellow-400 font-bold text-lg mt-1">⭐ {stars}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                        <span className="text-gray-500 text-xs uppercase tracking-wider">Forks</span>
                        <div className="text-green-400 font-bold text-lg mt-1">🍴 {body.forks ?? 0}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                        <span className="text-gray-500 text-xs uppercase tracking-wider">Commits</span>
                        <div className="text-blue-400 font-bold text-lg mt-1">☄️ {body.orbiting_particles}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                        <span className="text-gray-500 text-xs uppercase tracking-wider">Open PRs</span>
                        <div className="text-cyan-400 font-bold text-lg mt-1">🚀 {body.open_prs}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                        <span className="text-gray-500 text-xs uppercase tracking-wider">Issues</span>
                        <div className="text-amber-400 font-bold text-lg mt-1">🐛 {body.open_issues ?? 0}</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                        <span className="text-gray-500 text-xs uppercase tracking-wider">Mass</span>
                        <div className="text-orange-400 font-bold text-lg mt-1">🪐 {body.mass}</div>
                    </div>
                </div>

                {/* Updated At */}
                {body.updated_at && (
                    <div className="px-5 pb-3 text-xs text-gray-500 font-mono">
